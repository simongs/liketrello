...
ALTER TABLE `task_agile`.`card`
    ADD COLUMN `board_id` INT(11) NOT NULL DEFAULT 0 AFTER `id`;
ALTER TABLE `task_agile`.`card`
    ADD INDEX `fk_board_id_idx` (`board_id` ASC);

UPDATE `task_agile`.`card` c, `task_agile`.`card_list` cl
SET c.board_id = cl.board_id
WHERE c.card_list_id = cl.id;

ALTER TABLE `task_agile`.`card`
    ADD CONSTRAINT `fk_card_board_board_id`
...
ALTER TABLE `task_agile`.`activity`
    ADD COLUMN `ip_address` VARCHAR(64) CHARACTER SET utf8 COLLATE utf8_bin NOT NULL DEFAULT '''' AFTER `detail`;
...
ALTER TABLE `task_agile`.`attachment`
    ADD COLUMN `thumbnail_created` TINYINT(1) NOT NULL DEFAULT 0 AFTER `file_type`;

-- Add `cover_image` to `card`
ALTER TABLE `task_agile`.`card`
    ADD COLUMN `cover_image` VARCHAR(255) CHARACTER SET utf8 COLLATE utf8_bin NULL DEFAULT NULL AFTER `position`;
